...
# # Display data based on noise ratio category
# st.header("노이즈 비율에 따른 데이터 보기")
# noise_category = st.selectbox(
#     "노이즈 비율 카테고리 선택:", ["None", "low_noise", "norm_noise", "high_noise"]
# )
# target_value = st.selectbox("target값 선택 :", ["None"] + list(df["target"].unique()))

# selected_data = df

# # Filter by noise_category if selected
# if noise_category == "low_noise":
#     selected_data = selected_data[selected_data["noise_ratio"] <= low_noise_threshold]
# elif noise_category == "norm_noise":
#     selected_data = selected_data[
#         (low_noise_threshold < selected_data["noise_ratio"])
#         & (selected_data["noise_ratio"] <= norm_noise_threshold)
#     ]
# elif noise_category == "high_noise":
#     selected_data = selected_data[selected_data["noise_ratio"] > high_noise_threshold]

# # Filter by target_value if selected
# if target_value:
#     selected_data = selected_data[selected_data["target"] == target_value]
...
# # Calculate noise categories for each target value
# noise_data = (
#     df.groupby("target")["noise_ratio"]
#     .apply(
#         lambda x: pd.Series(
