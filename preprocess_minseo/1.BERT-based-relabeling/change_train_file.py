...
train.loc[train["ID"].isin(test_with_predictions["ID"]), "target"] = train.loc[
    train["ID"].isin(test_with_predictions["ID"]), "ID"
].map(test_with_predictions.set_index("ID")["target"])
